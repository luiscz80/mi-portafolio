...
$(document).ready(function () {
    ...
    function setupShowMore(containerSelector, itemSelector, buttonSelector, itemsToShow = 3) {

        const $container = $(containerSelector);
        if ($container.length === 0) return;

        const $items = $container.find(itemSelector);
        const $showMoreBtn = $container.find(buttonSelector);
        let currentIndex = itemsToShow;

        $items.each(function (index) {
            if (index >= itemsToShow) $(this).hide();
        });

        $container.on("click", buttonSelector, function () {
            let shown = 0;
            $items.each(function (index) {
                if (index >= currentIndex && shown < itemsToShow) {
                    $(this).fadeIn();
                    shown++;
                }
            });
    ...
    setupShowMore(".ability-content", ".service-item", ".show-more-btn");
    setupShowMore(".portfolio-content", ".portfolio-item", ".show-more-btn");
